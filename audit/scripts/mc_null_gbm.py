...
for rep in range(REPS):
    ...
    oos = {nm: walk_forward(ds, fac, cfg.wf, H) for nm, fac in
           (("ridge", RidgeForecaster), ("gbm", partial(GBMForecaster, purge=H)),
...
for col, side in [("ridge_cw_zero","up"),("ridge_cw_mean","up"),("ridge_cw_mean_nw","up"),
                  ("gbm_cw_zero","up"),("gbm_cw_mean","up"),("gbm_cw_mean_nw","up"),
                  ("mean_cw_zero","up"),("ridge_dm_zero","two"),("gbm_dm_zero","two")]:
...
d.to_csv(f"audit/mc_null_gbm_h{H}_b{BLOCK}.csv", index=False)
