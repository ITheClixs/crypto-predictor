...
def sign_rotation_test(
    ...
    """Randomisation test that the forecast's sign carries information about the outcome's.

    Both :func:`pesaran_timmermann` and its phase-averaged wrapper need an analytic variance,
    and neither has a valid one here. The first divides by ``n`` and so treats overlapping
    ``h``-step labels as independent. The second averages ``h`` phase statistics that are
    themselves dependent -- their return windows interleave and they come from the same fitted
    models -- and refers the unstandardised average to ``N(0, 1)``, which requires
    ``Var(sum_k S_k / h) = 1``. That does not hold and the covariance terms are not estimated.

    This test needs no variance at all. The statistic is the raw excess hit rate,
    ``P - P*``, computed on every observation. Its null distribution is generated by randomly
    rotating the forecast series against the outcome series: a circular shift preserves each
    series exactly -- its serial dependence, its marginal proportion of positive values, and
    hence ``P*`` -- while destroying the alignment between them, which is precisely the null.
    Overlap is then harmless, because it inflates the reference distribution by the same amount
    as it inflates the statistic.

    ``block`` sets the minimum rotation, so shifts small enough to leave labels overlapping
    with themselves are excluded; pass the forecast horizon.
    """
    yt = np.asarray(y_true, dtype=float)
    yp = np.asarray(y_pred, dtype=float)
    mask = np.isfinite(yt) & np.isfinite(yp)
    ...
    up_true = (yt > 0).astype(float)
    up_pred = (yp > 0).astype(float)
    if up_pred.min() == up_pred.max():  # forecast never changes sign
        return TestResult(float("nan"), float("nan"))

    p_y, p_x = up_true.mean(), up_pred.mean()
    hit_indep = p_y * p_x + (1 - p_y) * (1 - p_x)
    observed = float(np.mean(up_true == up_pred) - hit_indep)

    rng = np.random.default_rng(seed)
    shifts = rng.integers(lo, n - lo, size=n_draws)
    draws = np.empty(n_draws)
    for i, s in enumerate(shifts):
        draws[i] = float(np.mean(up_true == np.roll(up_pred, s)) - hit_indep)

    # Two-sided, with the +1 correction that keeps the p-value from ever being exactly zero.
    extreme = int(np.sum(np.abs(draws) >= abs(observed)))
    p_value = (1.0 + extreme) / (1.0 + n_draws)
